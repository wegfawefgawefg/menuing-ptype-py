...
AUDIO_SETTINGS_MENU_OPTIONS = ["Volume", "Music", "Sound Effects", "Back"]
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_DOWN:
                state.menu_selection += 1
                if state.menu_selection >= len(AUDIO_SETTINGS_MENU_OPTIONS):
                    state.menu_selection = len(AUDIO_SETTINGS_MENU_OPTIONS) - 1
            if event.key == pygame.K_RETURN:
                selection = AUDIO_SETTINGS_MENU_OPTIONS[state.menu_selection]
                match selection:
                    case "Video":
...
def draw(state, screen):
    ...
    for i, option in enumerate(AUDIO_SETTINGS_MENU_OPTIONS):
